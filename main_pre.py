...
with tab1:
    ...
    #     st_folium(m, width=800, height=500, key="mapa_t1")
    ...
    #     else:
    #         st.info("Sin datos climáticos.")

    #     st.subheader("Afluencia por Alcaldía")
    #     st.dataframe(df_aflu_resumen[['alcaldia', 'Afluencia_Estimada']].sort_values('Afluencia_Estimada', ascending=False), use_container_width=True)

# ==========================================
# PESTAÑA 2: NAVEGACIÓN (PUNTO A A B)
# ==========================================
# with tab2:
#     st.header("📍 Recomendación de Rutas Seguras")
    
#     col_inputs, col_info = st.columns([1, 2])
    
#     with col_inputs:
#         st.subheader("Configura tu viaje")
#         lugar_a = st.text_input("Punto de Origen (A)", "Pantitlán")
#         lugar_b = st.text_input("Punto de Destino (B)", "Polanco")
        
#         fecha_ruta = st.date_input("Fecha de Viaje", value=fecha_sel, key="fecha_t2")
#         hora_ruta = st.slider("Hora de Salida", 0, 23, hora_sel, key="hora_t2")
        
#         btn_ruta = st.button("Calcular Ruta más Segura")

#     with col_info:
#         st.info("Utiliza esta pestaña para encontrar el camino con menor score de riesgo histórico e infraestructura.")
#         if btn_ruta:
#             st.success(f"Calculando mejor ruta desde {lugar_a} hacia {lugar_b}...")
#             # Aquí irá la lógica del algoritmo de Dijkstra que planeamos
#             st.warning("Integración de grafos en proceso.")


#############################################################################################
# 1er intento real de la segunda pestaña
#############################################################################################

# # --- 1. INICIALIZACIÓN DE ESTADO (Fuera de las pestañas) ---
# if 'ruta_seleccionada' not in st.session_state:
#     st.session_state.ruta_seleccionada = None  # Guardaremos los datos de la ruta, NO el objeto mapa
# if 'tabla_comparativa' not in st.session_state:
#     st.session_state.tabla_comparativa = None
...
#     with col_a:
#         origen_label = st.selectbox("Origen (A)", options=list(dict_nodos.keys()), index=0, key="sel_origen")
#         p_a = (dict_nodos[origen_label]['lat_start'], dict_nodos[origen_label]['lon_start'])
#     with col_b:
#         destino_label = st.selectbox("Destino (B)", options=list(dict_nodos.keys()), index=min(100, len(dict_nodos)-1), key="sel_destino")
#         p_b = (dict_nodos[destino_label]['lat_start'], dict_nodos[destino_label]['lon_start'])

#     if st.button("🔍 Calcular 5 Mejores Rutas", key="btn_calc"):
#         with st.spinner("Procesando trayectorias..."):
#             try:
#                 # Calculamos y guardamos solo los datos (nodos de las rutas)
#                 k_rutas_data = list(islice(nx.shortest_simple_paths(G, p_a, p_b, weight='weight'), 5))
#                 st.session_state.ruta_seleccionada = k_rutas_data
                
#                 # Generamos los datos de la tabla
#                 resumen = []
#                 for idx, r in enumerate(k_rutas_data):
#                     d = sum(G.get_edge_data(r[i], r[i+1])['weight'] for i in range(len(r)-1))
#                     resumen.append({"Ruta": f"Ruta {idx+1}", "Distancia (m)": round(d, 2)})
#                 st.session_state.tabla_comparativa = pd.DataFrame(resumen)
                
#             except nx.NetworkXNoPath:
#                 st.error("No hay ruta disponible.")

#     # --- RENDERIZADO DEL MAPA (Fuera del botón para que persista) ---
#     if st.session_state.ruta_seleccionada:
#         # Re-creamos el objeto mapa en cada pasada para evitar errores de serialización
#         m_res = folium.Map(location=[p_a[0], p_a[1]], zoom_start=14, tiles='cartodbpositron')
#         colores = ['#007bff', '#28a745', '#ffc107', '#fd7e14', '#dc3545']
        
#         for idx, ruta in enumerate(st.session_state.ruta_seleccionada):
#             for i in range(len(ruta) - 1):
#                 u, v = ruta[i], ruta[i+1]
#                 data = G.get_edge_data(u, v)
#                 coords = [(lat, lon) for lon, lat in data['geometry'].coords]
#                 folium.PolyLine(coords, color=colores[idx], weight=6-idx, opacity=0.8).add_to(m_res)
        
#         folium.Marker(p_a, icon=folium.Icon(color='green')).add_to(m_res)
#         folium.Marker(p_b, icon=folium.Icon(color='red')).add_to(m_res)

#         # MOSTRAR RESULTADOS
#         c_map, c_tab = st.columns([3, 1])
#         with c_map:
#             # SOLUCIÓN AL BUCLE: returned_objects=[] evita que el mapa mande datos de vuelta y reinicie el script
#             # SOLUCIÓN AL WIDTH: usamos width='stretch' como pide tu consola de 2026
#             st_folium(
#                 m_res, 
#                 width='stretch', 
#                 height=500, 
#                 key="mapa_navegacion_estatico",
#                 returned_objects=[] 
#             )
#         with c_tab:
#             st.dataframe(st.session_state.tabla_comparativa, width='stretch')

#############################################################################################
# 2do intento real de la segunda pestaña
#############################################################################################
...
# with tab2:
#     st.header("📍 Recomendación de Rutas Seguras e Inteligentes")
...
#     # --- 2. LÓGICA DE CÁLCULO (Dentro del botón) ---
#     if st.button("🚀 Calcular Rutas con Riesgo Dinámico"):
#         with st.spinner("Analizando micro-riesgos..."):
#             try:
#                 # 1. Preparar factores dinámicos
...
#                     # Usamos los promedios globales calculados en T1
#                     score_total = (0.4 * avg_acc) + (0.3 * avg_inf) + (0.2 * s_lluvia) + (0.1 * s_aflu)
#                     d['costo_riesgo'] = d['weight'] * (1 + score_total)
#                     d['score_display'] = score_total

#                 # 3. Calcular las 5 rutas
#                 k_rutas = list(islice(nx.shortest_simple_paths(G, p_a, p_b, weight='costo_riesgo'), 5))
                
#                 resumen_list = []
#                 # Guardamos los nodos para reconstruir el mapa fuera del botón
#                 st.session_state.nodos_rutas = k_rutas 
                
#                 for idx, ruta in enumerate(k_rutas):
#                     dist_acum, risk_acum, count = 0, 0, 0
#                     for i in range(len(ruta)-1):
#                         data = G.get_edge_data(ruta[i], ruta[i+1])
#                         dist_acum += data['weight']
#                         risk_acum += data['score_display']
#                         count += 1
                    
#                     avg_r = (risk_acum / count) if count > 0 else 0
#                     resumen_list.append({
#                         "Ruta": f"Ruta {idx+1}",
#                         "Distancia (m)": round(dist_acum, 1),
#                         "Riesgo General": round(avg_r, 2),
#                         "Seguridad": "Alta" if avg_r < 0.35 else "Media" if avg_r < 0.6 else "Baja"
#                     })
                
#                 st.session_state.resumen_rutas = pd.DataFrame(resumen_list)

#             except nx.NetworkXNoPath:
#                 st.error("No se encontró ruta.")

#     # --- 3. RENDERIZADO PERSISTENTE (Fuera del botón) ---
#     if st.session_state.nodos_rutas is not None:
#         st.write("---")
#         c_map, c_tabla = st.columns([3, 1])
...
#             folium.Marker(p_a, icon=folium.Icon(color='green', icon='play')).add_to(m_res)
#             folium.Marker(p_b, icon=folium.Icon(color='red', icon='stop')).add_to(m_res)

#             # SINTAXIS 2026: width='stretch' y returned_objects=[] para evitar bucles
#             st_folium(m_res, width='stretch', height=550, key="mapa_final_t2", returned_objects=[])
...
#                 hide_index=True,
#                 column_config={
#                     "Riesgo General": st.column_config.ProgressColumn(min_value=0, max_value=1)
#                 }
#             )
...
#############################################################################################
# 4to intento real de la segunda pestaña
#############################################################################################

# def aplicar_distribucion_afluencia2(df, hora):
#     # Definimos picos: 8 AM (mu=8) y 6 PM (mu=18)
#     pico_1 = np.exp(-((hora - 8)**2) / (2 * 2**2)) 
#     pico_2 = np.exp(-((hora - 18)**2) / (2 * 3**2))
#     factor_hora = (pico_1 + pico_2) / 1.5 
    
#     # Agregamos por alcaldía
#     df_res = df.groupby('alcaldia')['afluencia'].mean().reset_index()
    
#     # --- SOLUCIÓN AL ERROR ---
#     # Llenamos los nulos con 0 antes de multiplicar y convertir a entero
#     df_res['afluencia'] = df_res['afluencia'].fillna(0)
    
#     df_res['Afluencia_Estimada'] = (df_res['afluencia'] * factor_hora).astype(int)
    
#     # Normalización 0-1 (con seguridad por si el máximo es 0)
#     max_val = df_res['Afluencia_Estimada'].max()
#     df_res['Afluencia_Norm'] = df_res['Afluencia_Estimada'] / max_val if max_val > 0 else 0
    
#     return df_res

# def obtener_penalizacion_horaria(hora):
#     """
#     Define un factor de recargo al riesgo según la hora del día.
#     - Madrugada/Noche (21-05): Riesgo Alto (Visibilidad/Velocidad)
#     - Pico Mañana (07-09) y Tarde (18-20): Riesgo Alto (Congestión)
#     - Valle (11-16): Riesgo Bajo
#     """
#     if 7 <= hora <= 9: return 0.25      # Pico Mañana
#     if 18 <= hora <= 20: return 0.30     # Pico Tarde
#     if 21 <= hora <= 23 or 0 <= hora <= 5: return 0.35 # Noche/Madrugada
#     if 11 <= hora <= 16: return 0.05     # Valle Mediodía (Reducción)
#     return 0.15                         # Horas de transición
...
# with tab2:
#     st.header("📍 Planeación de Rutas Seguras e Inteligentes")
    
#     # 1. Filtros Temporales Específicos
#     c1, c2 = st.columns(2)
#     with c1:
#         f_ruta = st.date_input("Día del viaje", value=datetime.date(2024, 1, 1), key="f_t2")
#     with c2:
...
#     col_a, col_b = st.columns(2)
#     with col_a:
#         origen_label = st.selectbox("Punto A (Origen)", options=list(dict_nodos.keys()), key="o_t2")
#         p_a = (dict_nodos[origen_label]['lat_start'], dict_nodos[origen_label]['lon_start'])
#     with col_b:
#         destino_label = st.selectbox("Punto B (Destino)", options=list(dict_nodos.keys()), key="d_t2")
#         p_b = (dict_nodos[destino_label]['lat_start'], dict_nodos[destino_label]['lon_start'])

#     if st.button("🚀 Calcular 5 Rutas con Riesgo Dinámico"):
#         with st.spinner("Sincronizando afluencia, clima y factores horarios..."):
#             try:
#                 # --- A. FILTRADO DE AFLUENCIA POR FECHA ANTES DE DISTRIBUIR ---
#                 df_aflu_fecha = df_aflu[df_aflu['fecha'].dt.date == f_ruta]
                
#                 # Si no hay datos para esa fecha exacta, usamos el histórico general (fallback)
#                 # if df_aflu_fecha.empty:
#                 #     df_aflu_fecha = df_aflu
                
#                 # df_aflu_ruta = aplicar_distribucion_afluencia2(df_aflu_fecha, h_ruta)
#                 # Si la fecha seleccionada no tiene datos, usamos el promedio histórico 
#                 # de ese mes para que la app no se rompa
#                 if df_aflu_fecha.empty:
#                     st.warning(f"No hay datos específicos para {f_ruta}. Usando promedios históricos del mes.")
#                     df_aflu_fecha = df_aflu[df_aflu['fecha'].dt.month == f_ruta.month]
                
#                 # Si sigue vacío (por si acaso), usamos todo el dataset
#                 if df_aflu_fecha.empty:
#                     df_aflu_fecha = df_aflu

#                 df_aflu_ruta = aplicar_distribucion_afluencia(df_aflu_fecha, h_ruta)
#                 mapa_aflu = df_aflu_ruta.set_index('alcaldia')['Afluencia_Norm'].to_dict()

#                 # --- B. CLIMA Y PENALIZACIÓN HORARIA ---
#                 df_cli_ruta = df_cli[(df_cli['time'].dt.date == f_ruta) & (df_cli['time'].dt.hour == h_ruta)]
#                 mapa_lluvia = df_cli_ruta.set_index('alcaldia')['precipitation_probability'].to_dict()
                
#                 s_hora = obtener_penalizacion_horaria(h_ruta)

#                 # --- C. ACTUALIZACIÓN DINÁMICA DE PESOS ---
#                 for u, v, d in G.edges(data=True):
#                     alc = d.get('alcaldia_name', 'CUAUHTEMOC')
#                     v_max = d.get('maxspeed', 40)
...
#                     # FÓRMULA MAESTRA: Ahora incluimos s_hora como factor de crecimiento
#                     # El riesgo base crece según la hora seleccionada
#                     score_g = (0.25 * avg_acc) + (0.15 * avg_inf) + (0.20 * s_vel) + \
#                               (0.15 * s_lluvia) + (0.10 * s_aflu) + (0.15 * s_hora)
                    
#                     # Normalización preventiva
#                     score_g = min(score_g, 1.0)
                    
#                     d['costo_seguro'] = d['weight'] * (1 + score_g)
#                     d['score_seg_segmento'] = score_g
#                     d['v_segmento'] = v_max

#                 # --- D. CÁLCULO DE LAS 5 RUTAS ---
#                 k_rutas = list(islice(nx.shortest_simple_paths(G, p_a, p_b, weight='costo_seguro'), 5))
#                 st.session_state.nodos_rutas = k_rutas
...
#             except nx.NetworkXNoPath:
#                 st.error("❌ No existe una ruta ciclista conectada entre estos puntos.")
...
#         with c_tab:
#             st.write("### 📋 Tabla de Seguridad Dinámica")
#             st.dataframe(
#                 st.session_state.resumen_rutas, 
#                 width='stretch', 
#                 hide_index=True,
...
#             )
#             # Leyenda informativa
#             st.caption(f"**Impacto Horario:** +{obtener_penalizacion_horaria(h_ruta)*100:.0f}% riesgo base.")